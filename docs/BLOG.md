...
Embedding：把离散的 token 编号，查表变成连续向量。
...
这些数字本身没有语义，只是编号。模型不能直接理解 "135"。所以需要 embedding 表：

```
embedding_table[135] = [0.12, -0.33, 0.08, ...]
embedding_table[246] = [-0.22, 0.51, 0.19, ...]
embedding_table[789] = [0.77, -0.10, 0.44, ...]
```

于是每个 token 都变成一个向量。

你可以把 embedding 理解成：每个词在 "语义地图" 上的坐标。

相似的 token，训练后通常会在向量空间里更接近。

隐藏维度是什么？和 embedding 有什么关系？

隐藏维度 hidden size / d_model，就是每个 token 向量有多少个数字。
...
意思是每个 token 会被表示成一个 4096 维向量：

```
token_embedding = [x1, x2, x3, ..., x4096]
```
...
如果词表大小是 `vocab_size = 50000`，隐藏维度是 `d_model = 4096`，那么 embedding 表大小就是：

```
50000 × 4096
```

也就是说，每个 token 都有一个 4096 维的可学习向量。
...
在 Transformer 里，每个 token 的隐藏向量 `x` 会通过三个 Linear 层变成 Q、K、V：
...
因为 "我用来提问的信息" "我用来被别人匹配的信息" "我真正传递出去的信息" 不一定应该是同一种表示。
